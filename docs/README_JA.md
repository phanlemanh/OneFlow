...
> **プラグインの認証情報はすべて「設定」にあります。** プラグインシステム自体は特定のプラットフォームに縛られず、どのプロバイダーもハードコードしていません。設定ダイアログは汎用的な環境変数の key/value エディタで、その値がプラグインに渡され、各プラグインがどのキーを必要とするかはそれぞれの README に記載されています。Director agent だけは唯一のファーストパーティ例外です——これはプラグインではなく TongFlow に組み込まれた機能で、常に Anthropic API を呼び出すため、`ANTHROPIC_API_KEY` を専用に必要とします。値はローカルに保存され、変更は即座に反映され、再起動は不要です。
