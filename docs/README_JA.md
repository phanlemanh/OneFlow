...
- **全モデル対応**: AIモデルは**モダリティ変換**として捉えることができます（例：LLMはテキスト→テキスト、画像モデルはテキスト→画像、音声モデルはテキスト→音声など）。OneFlow は各能力をノードとしてカプセル化します。

- **全モダリティ対応**: OneFlow は、Web上で実際に流通しているほぼすべてのモダリティとファイル形式をサポートします。

- **低い参入障壁、高い可能性**: 複雑なAIパラメータを学ぶ必要も、手動でノードを接続する必要もありません。**追加**、**変換**、**結合**の3つの操作だけで、自由に創意を組み立てられます。さらに、AIモデルを自由に編成することで、独自の創作物を生み出せます。

- **オープンなエコシステム**: OneFlow のプラグインベースの設計により、各プラットフォームが独立したプラグインをカプセル化でき、公式は各能力ノードに対して少なくとも1つの実装プラグインを提供します。コアは軽量に、エコシステムはオープンに。

## 実装済み機能

> ✅ = すぐに使える（公式プラグインあり）· ⬜ = まだ使えない——キャンバスノードか公式プラグインのどちらかが未提供（計画中）。
...
- ⬜ **感情 / 音色参照の音声合成**: テキストと参照音声 → その声色と感情で読み上げ。
- ⬜ **タイムスタンプ付き文字起こし**: 文ごとのタイムコード付き書き起こし——プラグインは対応済み、キャンバスノードが未提供。
...
**Node**（`pnpm` を含む）と、`PATH` 上に **Python 3.10+** インタープリタが必要です（`PYTHON` で特定のものを指定可能）。プラグインはローカルのPythonプロセスとして実行されます。OneFlow は自動的に各プラグイン用の隔離された venv を作成し、初回利用時に各プラグインの `requirements.txt` をインストールします——Pythonの手動設定は不要です。
...
セルフホストの OneFlow はプラグインを一切同梱しておらず、キャンバスにはサンプルワークフローがあらかじめ読み込まれています。以下の3ステップで動かせます：
...
**設定**（右上の歯車アイコン）を開き、プラグインが必要とする環境変数を入力します——たとえばAPIプラグイン用の `OPENAI_API_KEY` や、GPU/CPUプラグインに必要な認証情報など。**Director agent**（キャンバス上のきらめきアイコン——自然言語のプロンプトをワークフローグラフに変換します）を使うには、ここに `ANTHROPIC_API_KEY` も入力してください。

> **プラグインの認証情報はすべて「設定」にあります。** プラグインシステム自体は特定のプラットフォームに縛られず、どのプロバイダーもハードコードしていません。設定ダイアログは汎用的な環境変数の key/value エディタで、その値がプラグインに渡され、各プラグインがどのキーを必要とするかはそれぞれの README に記載されています。Director agent だけは唯一のファーストパーティ例外です——これはプラグインではなく OneFlow に組み込まれた機能で、常に Anthropic API を呼び出すため、`ANTHROPIC_API_KEY` を専用に必要とします。値はローカルに保存され、変更は即座に反映され、再起動は不要です。
...
OneFlow は、フォーク元の上流プロジェクトと同じ **[AGPL-3.0](../LICENSE)** でライセンスされています。
これには AGPL 第 13 条が含まれます：改変した OneFlow をネットワークサービスとして運用する場合、
そのサービスの利用者に完全な対応ソースを提供する必要があります。

本フォークは商用ライセンスを**付与できません**。デュアルライセンスの選択肢は上流の著作権者
tong-io にあります——非 AGPL 条件で TongFlow が必要な場合は、
[tong-io/tongflow](https://github.com/tong-io/tongflow) に直接お問い合わせください。
