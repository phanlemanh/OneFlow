...
> **⚠️ Partly superseded** by `refactor(abi): move all inline sourceSpec onto NODE_TYPE_SOURCE_SPEC`. The root cause described below — inline `sourceSpec` JSX props a server-side compiler cannot read — is gone: all 56 ABI node types now declare their overrides in `NODE_TYPE_SOURCE_SPEC`, and `abi/node-feature-registry.test.ts` fails the build if a component reintroduces one. Consequently:
>
> - `DIRECTOR_EXCLUDED_SLOTS` is now **empty**. Keep the set and its guard test — they still police the next slot the compiler cannot model — but Step 3's "populate it by running the guard test" yields no entries, because the guard demands none. Step 4's proof becomes two-directional: temporarily *add* a stale entry and watch the second test reject it; temporarily remove a registry entry and watch the first test demand an exclusion, naming the component file.
> - `LOCAL_SOURCE_SPEC_OVERRIDES` has been **deleted** from `compile.ts` — it mirrored 8 node types the registry now carries identically. Disregard the references to it in the Interfaces list and the Step 1 test sketch below: `sourceSpecOverridesFor` reads the registry directly, and the "modelled" set in `safe-slots.test.ts` is `Object.keys(NODE_TYPE_SOURCE_SPEC)` alone.
> - Step 5's polish item 3 is already implemented (`materialUntrackedFields` has the presence check, covered by "reports a readable message instead of throwing TypeError…"). It is worded as "the `LOCAL_SOURCE_SPEC_OVERRIDES` guard" below; the guard it means is `materialUntrackedFields` over `INLINE_SOURCE_SPEC_SWEEP`, which outlived the table and is now a secondary check behind `abi/node-feature-registry.test.ts`.
