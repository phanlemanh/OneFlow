...
| `repin-khong-chay-lai-eval` | T2 | 03/09 | *ngoài lộ trình* — hạ tầng cổng nghiệm thu: ghim lại một hồ sơ đã ký chạy lại bộ lệnh máy nhưng KHÔNG chạy lại phép kiểm của chính hồ sơ ấy, kể cả khi diff của lần ghim chạm đúng thứ chúng canh; không ai đo được vì dòng ghim không mang mốc cũ. Lần chạy thật đầu tiên tìm ra 6 ô bị nuốt, trong đó 1 ô chưa từng có dòng chạy nào. 14/14 eval, 7 giới hạn khai rõ |
...
| `skill-system-v1` | T3 | 17/09 | **B5** — khung skill của [ADR-0002](adr/0002-skill-template-orchestrator.md): một skill là template đồ thị kèm bảng tham số; người dùng bấm «Skill» ở thanh trái canvas, điền biểu mẫu sinh từ manifest, theo dõi từng bước, nhận đầu ra theo nhãn, và chỉ khi muốn mới mở «Xem/sửa kế hoạch» lên canvas. Máy chủ từ chối tham số sai, slot thiếu plugin, quá ba lượt và skill lạ mà không ghi một hàng task nào; task skill chỉ mang id, phiên bản và tham số, runner dựng lại instance lúc chạy và từ chối khi phiên bản đã đổi. Hai skill trên plugin cục bộ; skill thứ hai (tách tiếng khỏi video) vào registry mà không chạm engine, compiler hay exporter, đo bằng phép đối chiếu đường dẫn của chính commit thêm nó. 21/21 eval qua ba vòng verify, vòng 1 và 3 BLOCKED vì hạ tầng làn chấm rồi chạy lại riêng; ô chữ-trong-ngăn do người chấm. 17 giới hạn khai rõ, nặng nhất: đóng rồi mở lại ngăn khi lượt đang chạy làm lượt đó chạy lại |
<!-- roadmap-ledger:end -->

**Đọc được gì từ tỉ lệ này (đếm lại 23/09 trên 41 hồ sơ):** 19/41 là hạng mục trên lộ trình,
