  ...
  for (const line of lines) {
    ...
    if (m[1] === parts[depth]) {
      if (depth === parts.length - 1) {
        const rawVal = m[2].replace(/\s+#.*$/, '').trim();
        // A double-quoted YAML scalar carries escapes (`\"`, `\\`); the lane
        // hands this string to `bash -c` verbatim, so leaving the backslashes
        // in turns `--with "x"` into `--with \"x\"` and the command dies before
        // it runs (measured 08/09: repin-lane suite 6 exit 2 in 0.0s). Unescape
        // only for the double-quoted form; single-quoted YAML has no escapes.
        // Bóc nháy chỉ khi CẢ HAI đầu là cùng một dấu nháy — tức scalar YAML
        // thật sự được quote. Bản gốc của kit bóc một ký tự ở mỗi đầu VÔ ĐIỀU
        // KIỆN, nên một lệnh kết thúc bằng dấu nháy thuộc về CHÍNH LỆNH
        // (`pytest -k 'a or b'`) bị cắt mất dấu đóng → `bash -c` vỡ cú pháp,
        // thoát 2 trước khi chạy. Đo 10/09: 6 eval của `chong-doc-sai-em-ru`
        // chết đúng kiểu đó trong khi chạy tay thì xanh.
        const val = /^".*"$/.test(rawVal)
          ? rawVal.slice(1, -1).replace(/\\(["\\])/g, '$1')
          : /^'.*'$/.test(rawVal)
            ? rawVal.slice(1, -1)
            : rawVal;
