...
One divergence from precedent, flagged rather than buried. STATUS.md records
that at PR #18 this feature had to re-verify "vì nhánh đó chạm `sdk/**`" — a
whole-subtree reading of ownership. This re-pin uses the per-file reading the
rule in AGENTS.md actually states ("every differing file must belong to some
other feature"). Under it, the sdk files this branch adds — `engine/batch.py`,
`engine/callog.py`, `tests/conformance/**` — belong to conformance-l0, while
this feature's own sdk files (`pyproject.toml`, `tongflow/__init__.py`,
