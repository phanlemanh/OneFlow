...
- AC-11: Given `_acceptance/mo-hoa-b01/opportunity.md` khai `disposition: keep`, When chạy `scripts/fork/check-prototype-lane.sh mo-hoa-b01`, Then `prototype.base_commit` là tổ tiên của `main`, dòng `Diff: <base>...<tip>` có trong «Bảng nợ kế thừa», và mọi file của `git diff --name-only <base>...<tip>` có đúng một hàng trong bảng; thiếu file nào guard đỏ nêu tên file đó.
