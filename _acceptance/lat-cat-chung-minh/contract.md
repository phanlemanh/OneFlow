...
- AC-10: Given check-plan-freeze-teeth.sh, When chạy toàn bộ, Then 16/16 case xanh, mỗi case một mã thoát và một token `CASE <tên>: PASS`; tên case lạ bị từ chối exit 2; `--selftest-fail` in FAIL. **[amendment 04/09 — S4 vòng 1]** Con số ấy phải được đối chiếu với **độ dài mảng `CASES`**, không phải in `$pass/$pass`: hai vế bằng nhau bằng cấu trúc thì không bao giờ tố được một ca đã bị xoá. Ca thứ 16 là `o-moi-co-hoi-hong` — nửa fail-open của AC-2 mà 15 ca cũ không chạm.
