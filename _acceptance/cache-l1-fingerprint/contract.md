...
- AC-1: Given cùng một `(slot, pluginId, pluginRev, model, business_input)`, When `node_fingerprint()` được gọi ở **hai tiến trình khác nhau với `PYTHONHASHSEED` khác nhau**, Then hai khoá bằng nhau. *DoD của L1 là "fingerprint ổn định qua các lần chạy". `PYTHONHASHSEED` không đổi thứ tự lặp `dict` — `dict` đã insertion-ordered từ Python 3.7 và không hề bị hash-randomize; hạt giống đó chỉ ảnh hưởng thứ tự lặp `set`/`frozenset` và giá trị thô của `hash()`. Chạy ở hai tiến trình với hạt giống khác nhau bắt đúng loại lỗi thực tế: một khoá vô tình phụ thuộc `hash()`, `id()`, thứ tự lặp `set`, hoặc đồng hồ hệ thống — không cái nào trong số đó ổn định giữa các tiến trình.*
