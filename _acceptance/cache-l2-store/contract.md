...
- **Fixture test buộc phải `git init` thư mục plugin thật.** Fixture hiện tại là thư mục tay không có `.git`, nên `read_plugin_rev` trả `None` và theo luật L1 là không cacheable — test sẽ không bao giờ trúng cache. Đó cũng là cách duy nhất kiểm được AC-10.
