...
AC-14 và AC-16 viết nhãn dạng `- AC-14 **(cross-layer)**: Given …`, tức nhãn nằm **giữa**
id và dấu hai chấm. `eval-coverage-lint.js` khớp tiêu chí theo **từng dòng vật lý** bằng
`^\s*[-*]\s*(AC-\d+)\s*[:.]\s*(.+)$`, nên hai dòng này không khớp và **rơi khỏi tầm nhìn
của lint hoàn toàn**: W4 — tiêu chí gắn `(cross-layer)` phải có ≥1 eval khai
...
Chữ nghĩa tiêu chí giữ nguyên từng byte; chỉ vị trí nhãn đổi (diff: 2 dòng, 2+/2-).

**Đo lại sau khi sửa (07/08):** W4 im lặng vì cặp eval **thật sự có sẵn** — E14 (AC-14) và
E18 (AC-16) đều khai `layer: backend-effect`. Kiểm bằng phép thử răng: gỡ dòng `layer:`
của E14 → W4 đỏ đúng AC-14; trả lại → xanh. Vậy đây là lỗ hổng **bảo vệ**, không phải lỗ
hổng **bằng chứng**: evals đúng từ đầu, chỉ là không có gì canh chúng. Bằng chứng Cổng 2
đã ký không bị ảnh hưởng và chữ ký **không** cần cắt lại.
