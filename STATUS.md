...
| **S2** | **`local-cpu-plugins`** — `oneflow-api-ffmpeg` (6 slot) + `oneflow-api-pyscenedetect` (`SPLIT_VIDEO`) thay 2 plugin Modal `gpu=NONE`; venv riêng mỗi plugin; manifest 36 trơn + 3 origin | 🔜 **kế tiếp** — spec + plan xong, **chờ Cổng 1**. Chi phí ký lại tính bằng máy ở Cổng 1; chắc chắn có `per-plugin-origin` (guard manifest là AC-6 của nó) |
