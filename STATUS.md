...
- **Acceptance-Gate Kit:** đang chạy strict/strict (kit 1.31.0). **15 feature đã ký, tất cả trên `main`** — trục cache 1.1 đóng trọn (L0 conformance → L1 fingerprint → L2 store → L3 tier-B → L4 eviction), Phase 1.2 `compose-overlay` đã ship, hạng mục 0.6 (`gate-scope-anchors`) đã đóng. CI 5 job, gate là job thứ 5; `pre-merge-check: clean` xác nhận lại 05/08 trên `499b98e`.
...
- **Plugin fork đầu tiên đã hạ cánh:** `oneflow-modal-compose-overlay` là entry origin đầu tiên trong `official-plugins.json` (39 entry: 38 chuỗi trơn dưới org mặc định + 1 entry `{"id", "origin"}` dưới `phanlemanh`) — cơ chế per-plugin-origin từ lý thuyết thành đang chạy. Hai known-limit của per-plugin-origin thành LIVE với plugin này (nút "mở repo" + standalone SDK engine build URL từ org mặc định).
...
1. **Gói "khử mìn" (contract T2, làm TRƯỚC khi bump SDK kế tiếp):** (a) CI-vitest — 413 test vitest chưa có job CI nào chạy (rẻ, hiệu lực lớn; đụng workflows → `ci-actions-bump` ký lại, báo giá ở Cổng 1); (b) `run-overlay-plugin-tests.sh` hết ghi cứng `oneflow-sdk==0.2.18` — derive từ `sdk/pyproject.toml` như `check-overlay-sdk-train.sh`. **Lý do phải đi trước:** `normalize-text-vi` và TTS đều publish SDK mới; để nguyên thì 13 eval overlay_* xanh mà không chứng minh gì ngay lần bump đầu tiên; (c) CLAUDE.md §"Registering an official plugin" đang mô tả guard bản cũ (`expected_count`).
2. **`normalize-text-vi` (Phase 1.3):** slot tất định số/giá/ngày → chữ; bắt buộc đứng trước TTS trong mọi template; cũng là điều kiện phụ của ngưỡng MOS đề xuất trong [g0-runbook](docs/measure/g0-runbook.md). Kèm SDK 0.2.19 — đi qua đường guard vừa gỡ ở bước 1.
3. **Plugin TTS ElevenLabs (Phase 1.4, [ADR-0009](docs/adr/0009-tts-vi-eleven-v3.md)):** pattern API-plugin; entry origin thứ hai — cân nhắc mang theo "gói nợ lần fork đầu tiên" (mục Nợ).
4. **Skill system v1 (Phase 1.5, [ADR-0002](docs/adr/0002-skill-template-orchestrator.md)):** phần scaffold (template + manifest + orchestrator TS, `src/lib/skills/`) **không phụ thuộc ngách** — roadmap header ghi ngách chỉ đổi bộ thực thể KG + nguồn input skill P0. Bắt đầu được kể cả khi G0 chưa phán.
5. **Skill #1 + KG v0 (Phase 1.6–1.7):** chờ chốt ngách. Đo G1 ngay trong lúc build, không dồn về cuối.

Nợ xen kẽ giữa các feature lớn, theo giá: `overlay-canvas-reach` (hẹn ở Cổng 2 compose-overlay) · 1.1-L1b (mime vào digest) · 1.1-L2b (file_key_base) · extraction `run_workflow` · 0.7.
...
- **CI không chạy `pnpm test`** — phát hiện ở round 6: `.github/workflows/ci.yml` chạy lint/typecheck/build/sdk-pytest/pre-merge nhưng không vitest, nên **413 test** (compose-overlay vừa thêm ~66) và các guard mới chỉ có hiệu lực khi chạy tay local. Sửa = chạm workflows = chạm evidence của `ci-actions-bump` — đã xếp vào gói CI-a của Hàng đợi.
- **Known limits của `compose-overlay`** (chi tiết ở contract §Known limits): `run-overlay-plugin-tests.sh` **ghi cứng `oneflow-sdk==0.2.18`** — lần bump SDK tới, 13 eval overlay_* xanh mà không chứng minh gì (gỡ ở gói CI-a, TRƯỚC 1.3) · CLAUDE.md §"Registering an official plugin" mô tả guard bản cũ (`expected_count`) · `compose-overlay-op-form.tsx` dùng `<select>` thô, nhãn enum chưa dịch (5 locale thấy `top-left`, `tiktok-portrait`… tiếng Anh) · workflow nhập ngoài/sửa tay có thể còn dây video ở ô `logo` · schema ops gộp không ràng buộc field theo loại op (Amendment 2 — revisit khi generator hỗ trợ `oneOf`).
...
| **CI-a** | **Gói "khử mìn":** job vitest vào CI (413 test đang không được CI chạy) + `run-overlay-plugin-tests.sh` derive SDK pin từ `pyproject.toml` + CLAUDE.md §guard bản cũ | 🔜 **kế tiếp** — phải đi TRƯỚC lần bump SDK kế (normalize-text-vi/TTS đều publish SDK mới); đụng workflows → `ci-actions-bump` ký lại, báo giá ở Cổng 1 |
| **1.3** | Slot/node `normalize-text-vi` tất định (số, giá, ngày → chữ), đứng trước TTS trong mọi template | ⏭ sau CI-a — không phụ thuộc G0; kèm SDK 0.2.19 |
