...
**`cache-l3-tier-b` (lát L3, tầng B theo workflow) đang bay trên nhánh `feat/cache-l3-tier-b`, Cổng 1 ký 30/07.** Giá đã biết trước theo luật per-file: **ba** chữ ký lại — `cache-l1-fingerprint` (bump `KEY_SCHEMA_VERSION` 2→3, `workflowScope` vào khoá), `cache-l2-store` (sửa `node_cache.py` + cổng cache của `runner.py`), và `conformance-l0` (cùng `runner.py`/`engine-delegate.server.ts`) — mỗi chữ ký phải đứng trên bằng chứng chạy lại toàn bộ eval suite của feature đó trên cây L3 (AC-10/AC-12/AC-13 của contract L3).
...
| **1.1-L3** | Tầng B (memo theo workflow + tenant) | ◐ đang bay — nhánh feat/cache-l3-tier-b, ký Cổng 1 30/07 |
