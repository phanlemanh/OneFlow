...
- **A fourth coupled constant:** [`scripts/plugins/check-manifest-unmoved.sh`](scripts/plugins/check-manifest-unmoved.sh) asserts the manifest holds exactly **38 plain string entries** under the upstream org **plus exactly one origin entry** — `oneflow-modal-compose-overlay` under `phanlemanh`. It began as the `per-plugin-origin` feature's AC-6 (proof that adding the per-entry `origin` capability moved no plugin) and was rewritten when the first fork landed (compose-overlay, 2026-08-03), so it is a snapshot of those two PRs, not a standing invariant. Registering a 39th plugin, or writing a **second** `{"id": ..., "origin": ...}` entry, turns it red — edit the counts inside the guard's `node -e` block (there is no `expected_count` variable any more), or retire the guard, rather than wondering why an unrelated PR failed an eval.
