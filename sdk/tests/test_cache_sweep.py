...
plus (bottom of file) Task 4's runner-wiring tests for `reuse`, the cache
counters, the `node_cached` emission, and the end-of-run sweep (AC-9..12,
AC-16).
...
from .cache_helpers import _two_node_workflow
...
# --------------------------------------------------------------------------
# Task 4: runner wiring -- reuse=auto/off, cache counters, node_cached
# emission, end-of-run sweep (AC-9..12, AC-16). Fixtures follow
# `cache_helpers.py` / `test_node_cache.py`'s style: real `run_workflow`
# calls through a fake invoker, never a hand-built cache entry.
# --------------------------------------------------------------------------
...
def _snapshot(root: Path) -> dict[str, int]:
    """relpath -> size map of every file under `root`, or `{}` if absent."""
    if not root.exists():
        return {}
...
def test_reuse_off_touches_nothing(tmp_path):
    ...
    # on-disk tree byte-identical, and still return correct results.
